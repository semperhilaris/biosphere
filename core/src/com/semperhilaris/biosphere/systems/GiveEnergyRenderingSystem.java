 ...
 *
 */
public class GiveEnergyRenderingSystem extends SortedIteratingSystem {
	...
	public GiveEnergyRenderingSystem(Camera camera, PolygonSpriteBatch spriteBatch) {
		super(Family.all(GiveEnergy.class, Position.class).get(), new RenderOrderComparator());
		...
		texture = new Texture(Gdx.files.internal("textures/energy_beam.png"));
	...
	protected void processEntity(Entity entity, float deltaTime) {
		...
		GiveEnergy giveEnergy = giveEnergyComponentMapper.get(entity);

		if (giveEnergy.target != null) {
			Position targetPosition = positionComponentMapper.get(giveEnergy.target);
			if (targetPosition != null) {
				...
				Rotation rotation = rotationComponentMapper.get(giveEnergy.target);
				if (rotation != null) {
